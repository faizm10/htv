// Example component showing how to use the new database structure with autofill
'use client';

import React, { useState, useEffect } from 'react';
import { useDatabaseData, getAutofillSuggestions } from '@/lib/use-database-data';
import { Conversation, User, Message } from '@/lib/database-types';

interface ChatWithAutofillProps {
  conversationId: string;
}

export function ChatWithAutofill({ conversationId }: ChatWithAutofillProps) {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500">
              {conversation.metrics.responseRate * 100}% response rate
            </span>
          </div>
        </div>
      </div>
            ...
            <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
              ...
              <div className={`text-xs mt-1 flex items-center gap-2 ${
                ...
                {message.analysis.quality === 'dry' && (
                  <span className="text-orange-400">⚠️ Dry</span>
                )}
                {message.analysis.requiresResponse && (
                  <span className="text-yellow-400">📝 Response needed</span>
                )}
              </div>
            </div>
          </div>
...
export default ChatWithAutofill;
