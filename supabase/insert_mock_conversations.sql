-- Insert Mock Conversations Script for Supabase
-- This script adds Laura & Faiz and Smith & Wosly conversations with realistic data

-- Create variables for user IDs (using gen_random_uuid())
DO $$
DECLARE
    laura_uuid UUID := gen_random_uuid();
    faiz_uuid UUID := gen_random_uuid();
    smith_uuid UUID := gen_random_uuid();
    wosly_uuid UUID := gen_random_uuid();
    current_user_uuid UUID := gen_random_uuid();
    
    conv_laura_faiz_uuid UUID := gen_random_uuid();
    conv_smith_wosly_uuid UUID := gen_random_uuid();
    conv_current_laura_uuid UUID := gen_random_uuid();
BEGIN
    -- Insert users
  ...
  '{
    ...
    -- Insert messages for all conversations
